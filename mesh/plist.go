...
// decodePlist parses an Apple XML plist document and returns its root value the
// way plistlib.loads would. An empty or malformed document is an error, not a
// silent nil.
...
// decodePlistValue reads the next value element under the cursor (the opening tag
// of a plist value) and returns the decoded Go value.
...
// decodeDict reads a <dict> body of alternating <key> and value elements into a
// map, until its closing tag.
...
// decodePlistData decodes a <data> element's base64 body, dropping the whitespace
// Apple wraps it in so the returned bytes are the decoded content, not the base64
// source.
...
// elementText reads the character data of the element under the cursor and
