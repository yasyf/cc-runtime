...
func TestComposePairPayload(t *testing.T) {
	urls := []string{"http://192.168.1.5:8765", "https://host.tail1234.ts.net:25443"}
	p, raw, err := ComposePairPayload("mac-studio", urls, "deadbeef")
	if err != nil {
	...
	want := `{"v":1,"name":"mac-studio","urls":["http://192.168.1.5:8765",` +
		`"https://host.tail1234.ts.net:25443"],"token":"deadbeef"}`
	if raw != want {
	...
	if back.V != 1 || back.Name != "mac-studio" || back.Token != "deadbeef" {
